...
def page_not_found(e):
    ...
    #404를 생략하면 404.html 템플릿이 랜더링되어 오류페이지가 나타나지만 클라이언트는 200코드(정상일때 발생하는 코드)를 수신할 것
...
def create_app():
    app = Flask(__name__)
    app.config.from_envvar('APP_CONFIG_FILE') #환경 변수 APP_CONFIG_FILE에 정의된 파일을 환경 파일로 사용하겠다.
